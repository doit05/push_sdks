...
func NewMiPushClient(appSecret string, packageName []string) *MiPush {
	return &MiPush{
...
func (m *MiPush) SendToList(ctx context.Context, msg *Message, regIDList []string) (*SendResult, error) {
	if len(regIDList) == 0 || len(regIDList) > 1000 {
		return nil, fmt.Errorf("wrong number regIDList")
	}
...
//上传大图
func (m *MiPush) UploadImg(ctx context.Context, imgUrl string) (*MediaResult, error) {
	fileName := common.GetImgNameFromUrl(imgUrl)
	if fileName == "" {
		return nil, nil
	}
	imgUrl += "?x-oss-process=image/resize,limit_0,m_fill,l_876,s_324/quality,q_100"
	path := os.TempDir()
	filePath, err := common.DowlodPic(imgUrl, path, "", fileName)
	if err != nil {
		return nil, err
	}
	bytes, err := m.doUpload(ctx, m.host+MediaUploadImageURL, filePath, fileName)
	if err != nil {
		return nil, err
	}
	tmp := string(bytes)
...
func (m *MiPush) doUpload(ctx context.Context, uploadUrl, filePath, fileName string) ([]byte, error) {
	body := &bytes.Buffer{}
	writer := multipart.NewWriter(body)
	writer.WriteField("is_global", "false")
	writer.WriteField("is_icon", "false")
...
tryAgain:
	...
	if res.Body == nil {
		return nil, fmt.Errorf("xiaomi response is nil")
	}
	defer res.Body.Close()
	...
	if res.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("network error:[%v]", string(result))
	}
	return result, nil
}
